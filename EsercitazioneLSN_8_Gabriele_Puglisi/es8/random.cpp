...
double Random :: Exp(double mean) {
   // This function generates a random number from a Gaussian distribution with given mean and sigma
   double s=Rannyu();
   return -mean*log(1-s);         // si fa ln(1-y) per avere argomento tra (0,1]
}
...
double Random :: Cauchy(double x0, double gamma) {
   // This function generates a random number from a Gaussian distribution with given mean and sigma
   double s=Rannyu();
   return x0 + gamma * tan(M_PI * (s - 0.5)); // Calcola il valore corrispondente alla distribuzione di Cauchy
}



double Random :: UnifTheta() {   // estraggo unif tra 0 e pi senza conoscere pi
