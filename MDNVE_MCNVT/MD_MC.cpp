...
void Measure() //Properties measurement
{
  ...
  double vij;
  double dx, dy, dz, dr, pij;
  double p=0;

  for (int i = 0; i < nbins; i++) {
        walker[5 + i] = 0;  //azzerro i contatori sui bin
    }
...
// distance i-j in pbc
      ...
      if (dr < box / 2.) {  // Calculate g(r) if the particle distance is within half the box size
                sd = floor(dr / bin_size);
                walker[5 + sd] = walker[5 + sd] + 2;
            }
  ...
  //walker[ip] = rho*walker[it] + 48./(3.*vol)*p; // Pressure
  walker[ip] = rho*walker[it] + 48.*p/(3.*vol*(double)npart) + 32.*M_PI*( rho/vol )*( 1./( 9.*pow( rcut, 9 ) ) - 1./( 6.*pow( rcut, 3 ) ) ); //Pressure with tail corrections
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    for(int i=0; i < nbins; i++){                //update g(r) bins

      norm = 1. / ( rho * (double) npart *  (4.*M_PI/3.) * ( pow((double) (i+1) * bin_size, 3) - pow((double)i*bin_size, 3)) ) ;

      stima_gdir = blk_av[5 + i]*norm/blk_norm; //*norm;
      glob_av[5 + i] += stima_gdir;
      glob_av2[5 + i] += stima_gdir*stima_gdir;
    ...
    if(iblk==nblk){       //Print last g(r) on separate file

      for(int i=0; i < nbins; i++){

        err_gdir=Error(glob_av[5+i],glob_av2[5+i],nblk);
        Gdir << setw(18) << (double) i * bin_size << setw(18) << glob_av[5+i]/(double)nblk << setw(18) << err_gdir << endl;
      }
    }
    ...
    stima_pres = blk_av[ip]/blk_norm; //Temperature
